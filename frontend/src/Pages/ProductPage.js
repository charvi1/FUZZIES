...
const ProductsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:2151'}/api/products`, {
  ...
  const handleAddToCart = async (product) => {
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:2151'}/api/cart/addToCart`, {
        email: user.email,
  ...
  const handleFeedbackSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL || 'http://localhost:2151'}/api/products/${modalProduct._id}/feedback`,
