...
const CartPage = () => {
    ...
    const fetchUserCart = async () => {
        try {
            const response = await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:2151'}/api/cart`, { email });
    ...
    const clearCart = async () => {
        try {
            const response = await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:2151'}/api/cart/clearCart`, { email });
            if (response.data.success) {
    ...
    const handleToken = async (token) => {
        try {
            ...
            const response = await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:2151'}/api/payment/checkout`, {
                token: token.id,
            ...
            if (response.status === 200 && response.data.success) {
                ...
                // Create Order on Backend (Since payment is successful)
                const orderResponse = await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:2151'}/api/orders`, {
                    email: user.email,
                    shippingAddress: shipmentDetails,
                    totalAmount: totalAmount
                });

                if (orderResponse.data.success) {
                    // Start Frontend Cleanup (Backend already cleared the DB cart)
                    setCart([]);
                    setTotalAmount(0);
    ...
    const removeFromCart = async (productId) => {
        try {
            const response = await axios.delete(`${process.env.REACT_APP_API_URL || 'http://localhost:2151'}/api/cart/removeFromCart`, {
